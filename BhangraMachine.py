...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and ("Drum" in str(filepath))  and ("Bhangra" in str(filepath)): 
            if int(os.stat(filepath).st_size) < sizlim:
                contentbeats.append(filepath)

        if  filepath.endswith(".wav") and ("String" in str(filepath))  and ("Bhangra" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
        ...
        if  filepath.endswith(".wav") and ("One Shot" in str(filepath)) and ("Tabla" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:  
