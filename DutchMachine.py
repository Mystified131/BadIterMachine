...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and ("Drum" in str(filepath))  and ("Industrial" in str(filepath)): 
            if int(os.stat(filepath).st_size) < sizlim:
                contentbeats.append(filepath)

        if  filepath.endswith(".wav") and ("Drums" in str(filepath))  and ("Glitch" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
        ...
        if  filepath.endswith(".wav") and  ("NASA" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:  
