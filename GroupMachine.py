...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and ("Dubstep" in str(filepath)) and ("Drums" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:
                contentbeats.append(filepath)

        if  filepath.endswith(".wav") and ("Triangle" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
        ...
        if  filepath.endswith(".wav") and ("Trumpet" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
             contentorg.append(filepath)

        if  filepath.endswith(".wav") and ("Accordion" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
                contentsax.append(filepath)

        if  filepath.endswith(".wav") and  ("Bongo" in str(filepath))  :
            if int(os.stat(filepath).st_size) < sizlim:  
                contentgit.append(filepath)

        if  filepath.endswith(".wav") and ("Didg" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:  
