...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and (("Beat" in str(filepath)) or ("Drum" in str(filepath)))  and ("MJL" in str(filepath)): 
            if int(os.stat(filepath).st_size) < sizlim:
                contentbeats.append(filepath)

        if  filepath.endswith(".wav") and ("Rhythm" in str(filepath))  and ("MJL" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
                contentdrones.append(filepath)

        if  filepath.endswith(".wav") and ("Bass" in str(filepath)) and ("MJL" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
        ...
        if  filepath.endswith(".wav") and ("Guitar" in str(filepath)) and ("MJL" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
                contentsax.append(filepath)

        if  filepath.endswith(".wav") and  ("Soundscape" in str(filepath)) and ("MJL" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
                contentgit.append(filepath)

        if  filepath.endswith(".wav") and ("Rhodes" in str(filepath)) and ("MJL" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:  
