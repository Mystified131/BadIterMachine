...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav") and (("Pad" in str(filepath)) and ("Score" in str(filepath))):
            if int(os.stat(filepath).st_size) < sizlim:  
        ...
        if  filepath.endswith(".wav") and ("Drums" in str(filepath)) and ("Tribal" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:  
             contentorg.append(filepath)

        if  filepath.endswith(".wav") and ("String" in str(filepath)) and ("Score" in str(filepath)):
            if int(os.stat(filepath).st_size) < sizlim:  
                contentsax.append(filepath)

        if  filepath.endswith(".wav") and  ("Pad" in str(filepath)) and ("Glitch" in str(filepath)) :
            if int(os.stat(filepath).st_size) < sizlim:  
                contentgit.append(filepath)

        if  filepath.endswith(".wav") and ("Score" in str(filepath)) and ("Fx" in str(filepath))  :
            if int(os.stat(filepath).st_size) < sizlim:  
